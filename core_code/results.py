...
def read_images(tr, te):
    ...
    Y_tr = np.zeros((len_tr, IMG_HEIGHT, IMG_WIDTH,1), dtype=np.bool)
    
    X_te = np.zeros((len_te, IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS), dtype = np.uint8)
    Y_te = np.zeros((len_te, IMG_HEIGHT, IMG_WIDTH,1), dtype=np.bool)
    ...
    for i in tqdm(np.arange(len_tr)):
        ...
        X_tr[i] = img
        label = imread(Ypath)
        label = np.expand_dims(resize(label, (IMG_HEIGHT, IMG_WIDTH), mode = 'constant', 
...
if __name__ == '__main__':
    ...
    # load models
    #best_models = load_model('Ubestmodel.h5') # U_net
    best_models = load_model('Segbestmodel.h5') # Seg_net
    ...
    Y_rte = np.zeros((len(gls), IMG_HEIGHT, IMG_WIDTH), dtype=np.bool)
    ...
    preds_test = best_models.predict(X_rte, verbose=1) # Only need to use those four images not augmented images
    ...
    for i in np.arange(len(Y_rte)):
        pas[i] = pixel_accuracy(np.squeeze(preds_test_t[i]), Y_rte[i])
        mas[i] = mean_accuracy(np.squeeze(preds_test_t[i]), Y_rte[i])
        mious[i] = mean_IOU(np.squeeze(preds_test_t[i]), Y_rte[i])
        fwious[i] = frequency_weighted_IOU(np.squeeze(preds_test_t[i]), Y_rte[i])
    ...
    for i in np.arange(len(Y_rte)):
        ...
        imshow(np.squeeze(Y_rte[i])) # ground truth images
        imshow(np.squeeze(preds_test_t[i])) # nn trained result
        plt.show()